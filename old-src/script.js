...
class EPUBBook {
	...
	fixBookLanguage() {
		...
		try {
			...
			if (language_tags.length === 0) {
				...
				opf.getElementsByTagName("metadata")[0].appendChild(
					language_tag,
				);
			} else {
			...
			if (language !== original_language) {
				this.files[opf_filename] =
					new XMLSerializer().serializeToString(opf);
				this.fixedProblems.push(
	...
	async writeEPUB() {
		...
		if ("mimetype" in this.files) {
			await writer.add(
				"mimetype",
				new zip.TextReader(this.files["mimetype"]),
				{ level: 0 },
			);
		}
		...
		for (const file in this.binary_files) {
			await writer.add(
				file,
				new zip.Uint8ArrayReader(this.binary_files[file]),
			);
		}
